...
so unlike the stat graphics it is not regenerated on a schedule. Run this after
every rebuild of ascii.svg.
...
Consolas is about 0.55 -- would otherwise see the portrait roughly 7% too
narrow. Inlining pins it for everyone. An external font URL is not an option:
the SVG is loaded through <img>, and browsers refuse subresource fetches for
image documents.
...
def main():
    ...
    if not os.path.exists(FONT):
        raise SystemExit(f"{FONT}: missing -- run scripts/subset_fonts.py first")
