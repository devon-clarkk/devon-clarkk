...
Env:
  ...
  GH_LOGIN      user to summarise (default: devon-clarkk)   # EDIT-ME via secret
  OUT_DIR       where to write (default: repository root)

Flags:
  --demo        skip the API and draw from sample data, so the page looks
                complete before the first real Action run. The committed
                numbers are FAKE until the scheduled job overwrites them.
"""
import argparse
import base64
import functools
...
# ------ the one easy-to-change identity constant -------------------------
# The workflow overrides this with ${{ github.repository_owner }}, so it
# self-corrects to whatever account the repo lives under. This default is
# only used when you run the script by hand without GH_LOGIN set.
DEFAULT_LOGIN = "Devon-Clarkk"                                  # EDIT-ME

# The section headings drawn as SVGs. subset_fonts.py reads this list to decide
...
def demo_summary():
    """A deterministic sample year, so the four data SVGs exist and look real
    before the first scheduled run. No network, no token, same code path for
    drawing. Anchored to a fixed Sunday so re-running never commits noise."""
    start = date(2025, 8, 10)                 # a Sunday -> weekday 0
    weeks = []
    for wi in range(53):
        wk = []
        for wd in range(7):                   # 0=Sun .. 6=Sat, as GitHub returns
            d = start + timedelta(days=wi * 7 + wd)
            hump = 1.0 + 1.3 * math.sin(wi / 53.0 * math.pi)   # mid-year swell
            weekend = 0.4 if wd in (0, 6) else 1.0
            cnt = int((2 + (wi * 3 + wd * 5) % 6) * weekend * hump)
            if (wi * 7 + wd) % 11 == 0:       # a scattering of rest days
                cnt = 0
            wk.append(dict(contributionCount=cnt, date=d.isoformat(), weekday=wd))
        weeks.append(wk)
    days = [d for w in weeks for d in w]
    weekly = [sum(d["contributionCount"] for d in w) for w in weeks]
    cur, best = streaks(days)
    by_size = [("Python", 486000), ("TypeScript", 331000), ("JavaScript", 152000),
               ("CSS", 58000), ("Shell", 21000)]
    by_repo = [("Python", 6), ("TypeScript", 4), ("JavaScript", 2),
               ("Dockerfile", 1), ("Shell", 1)]
    return dict(
        total=sum(weekly), active=sum(1 for d in days if d["contributionCount"] > 0),
        best_week=max(weekly), weekly=weekly, weeks=weeks,
        current=cur, longest=best, by_size=by_size, by_repo=by_repo)
...
def draw_streak(s):
    ...
    for k, lab in (("current", "current streak"), ("longest", "longest streak")):
        r = s[k]
        # plain hyphen and a word, not en/em dashes: keeps the glyphs inside the
        # basic-latin subset and honours the no-em-dash preference.
        span = (f"{pretty(r['start'])} - {pretty(r['end'])}"
                if r["length"] else "none yet")
...
def main():
    ...
    ap.add_argument("--demo", action="store_true",
                    help="draw from sample data, no token/network needed")
    args = ap.parse_args()
    ...
    if args.demo:
        print("=" * 68)
        print("  --demo: drawing SAMPLE stats. The committed numbers are FAKE")
        print("  until the scheduled Action (or a manual 'Run workflow') runs.")
        print("=" * 68)
        s = demo_summary()
        login = os.environ.get("GH_LOGIN", DEFAULT_LOGIN)
    else:
