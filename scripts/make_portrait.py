...
Run it once; it is not on a schedule, unlike scripts/generate_stats.py.
    ...
    python3 scripts/make_portrait.py assets/photo.jpg --crop 400,110,910,790
    python3 scripts/embed_portrait_font.py      # inline the font, see below

The first run downloads a ~176 MB background-removal model, once. If you do not
want that dependency (for example, your photo already sits on a clean white
background, like the shipped placeholder), pass --no-remove-bg and no model is
fetched.

A note on the ramp. The README describes the "quiet to loud" ramp as `:` `+`
`#` `@`. That four-step ramp is the stepped version year.svg draws with; the
portrait itself uses the fuller thirteen-step RAMP below, because a four-level
face reads as broken. Both ship. RAMP is a plain constant -- edit it and the
font subset follows automatically (scripts/subset_fonts.py reads it).

Two things decide whether the output is any good, and neither is a parameter:

  * The photo. ASCII draws with shadow, not detail -- about 13 brightness
    levels in total. You need side light (a window at ~45 degrees, everything
    else off), a tight crop from chin to just above the hair, and real
...
def _even_and_darken(gray, alpha, curve=CURVE, clahe=CLAHE_CLIP):
    """Even the local contrast with CLAHE, then darken. cv2 if present, else a
    Pillow fallback so the pipeline still runs without opencv installed."""
    try:
        import cv2
        ...
        print("note: opencv not available, using a plainer Pillow contrast pass "
              "(install opencv-python-headless for the intended look)",
              file=sys.stderr)
        gray = np.array(ImageOps.autocontrast(Image.fromarray(gray), cutoff=1))
    gray = (255.0 * (gray / 255.0) ** curve).astype("uint8")
    gray[alpha < 20] = 255                                 # force the matte to white
    return Image.fromarray(gray)


def prep(path, crop=None, remove_bg=True, curve=CURVE, clahe=CLAHE_CLIP):
    ...
    if remove_bg:
        # Lazy import: only pull in rembg (and its ~176 MB model) when asked.
    ...
    return _even_and_darken(gray, alpha, curve=curve, clahe=clahe)
...
def main():
    ...
    ap.add_argument("--curve", type=float, default=CURVE,
                    help=f"darkening exponent (default {CURVE}); lower = lighter, "
                         f"try ~1.2 for softer/frontal light")
    ap.add_argument("--clahe", type=float, default=CLAHE_CLIP,
                    help=f"local-contrast clip limit (default {CLAHE_CLIP}); "
                         f"lower = less skin-texture noise")
    ap.add_argument("--gamma", type=float, default=GAMMA,
                    help=f"ramp mapping exponent (default {GAMMA})")
    ap.add_argument("--no-remove-bg", dest="remove_bg", action="store_false",
                    help="skip rembg background removal (and its model download); "
                         "use when the photo already has a clean white background")
    ...
    lines = to_lines(prep(args.photo, crop, remove_bg=args.remove_bg,
                          curve=args.curve, clahe=args.clahe),
                     cols=args.cols, gamma=args.gamma)
    if args.preview:
        print("\n".join(lines))
